...
- In case, token creation is successful, ResponseDto will have SUCCESS status, Jwt will be set as Cookie into headers and returned along with 201 status code.
...
- Please create a bean of SecretKey using `secretKeyString` whose value is present in application.properties while generating signature. 
- Make sure to use HS256 while generating signature
